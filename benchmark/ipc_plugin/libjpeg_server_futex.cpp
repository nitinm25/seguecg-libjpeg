...
static void server_run(ipc_control* ctrl) {
    while (true) {
        // Wait for client to post a request
        while (futex_load(&ctrl->status, std::memory_order_acquire) != FUTEX_REQUEST) {
            futex_wait(&ctrl->status, FUTEX_IDLE);
        }
...
#endif
                futex_store(&ctrl->status, FUTEX_RESPONSE, std::memory_order_release);
